...
def dLQR(F,Q,R,x,xf,dt=0.1):
    ...
    for t in range(N, 0, -1):
      ...
      # state cost matrix
      P_lqr[t-1] = Q + A.T @ P_lqr[t] @ A - (A.T @ P_lqr[t] @ B) @ la.pinv(
               R + B.T @ P_lqr[t] @ B) @ (B.T @ P_lqr[t] @ A)      
    ...
    for t in range(N):
      ...
      K_lqr[t] = -la.pinv(R + B.T @ P_lqr[t+1] @ B) @ B.T @ P_lqr[t+1] @ A
...
def get_Q():
    ...
    Output
      :return: Q: State cost matrix (3x3 matrix because the state vector is 
                  (X, Y, THETA))
    """
    Q = np.array([[1, 0, 0], # Penalize X position error (global coordinates)
                  [0, 40, 0], # Penalize Y position error (global coordinates)
                  [0, 0, 28]])# Penalize heading error (global coordinates)
    #Q = np.array([[1, 0, 0], # Penalize X position error (global coordinates)
    #              [0, 36, 0], # Penalize Y position error (global coordinates)
    #              [0, 0, 28]])# Penalize heading error (global coordinates)
